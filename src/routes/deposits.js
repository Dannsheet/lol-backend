...
router.post("/deposits/webhook", async (req, res) => {
  try {
    ...
    if (rpcError) {
      ...
      if (
        msg.includes("Falta txHash") ||
        msg.includes("amount inválido") ||
    ...
    if (!row?.ok) {
      return res.status(500).json({ error: "No se pudo procesar el depósito" });
    }
    ...
    return res.json({
      ok: true,
      credited: row.credited ?? amountStr,
